...
2. **Расчет приростов** по двум алгоритмам:
...
5. **Экспорт результатов** в Excel файл с тремя листами
...
#### TEST_DATA_CONFIG
- `create_test_data`: Создавать тестовые данные при запуске (True/False)
...
#### Функции:
- `check_and_create_test_data()`: Проверка и создание тестовых данных при необходимости
- `main()`: Главная функция программы с логикой создания тестовых данных
...
### Лист "Клиентские менеджеры" (по дате отчета)
...
### Лист "КМ по дате сделки" (по дате сделки)
Содержит сводку по менеджерам с расчетом по менеджеру в каждом периоде:
- Табельный уникальный
- ФИО
- ТБ, ГОСБ
- Показатели по периодам (суммы по клиентам закрепленным в каждом периоде)
- Суммарный прирост
